...
def create_dummy_descriptor_from_address(addr: Optional[str]) -> 'Descriptor':
    ...
    def guess_script_type(addr: Optional[str]) -> str:
        # Default to legacy on coins without SegWit
        if addr is None:
            return 'p2pkh'

        # Try legacy Base58 first (works regardless of SegWit support)
        try:
            addrtype, _h160 = bitcoin.b58_address_to_hash160(addr)
            if addrtype == constants.net.ADDRTYPE_P2PKH:
                return 'p2pkh'
            if addrtype == constants.net.ADDRTYPE_P2SH:
                # Keep Electrum’s conservative estimate for P2SH (nested wpkh) for size calc;
                # if you prefer, you can return 'p2sh' but then also add support for it below.
                return 'p2wpkh-p2sh'
        except Exception:
            # not base58; maybe bech32 if SegWit is actually enabled
            pass

        # Only attempt Bech32/SegWit if HRP is configured (non-empty) and addr matches it
        hrp = getattr(constants.net, 'SEGWIT_HRP', None)
        if isinstance(hrp, str) and hrp and addr.lower().startswith(hrp.lower() + '1'):
